...
function priceSwitch() {
  ...
  items.forEach(el => {
    ...
    $(document).on('click', '.getcountry', function () {

        $('#country-dropdown').hide();
        let countryName = $(this).find('.js-search-option-target').text();
        let countryCode = $(this).find('.country-id').text().split(',')[0]; // Get the code from text

        let datalist = [];
        // Fetch cities for the selected country
        $.ajax({
            url: location.origin + "/NamoriTravels/GetCities",
            method: 'GET',
            ...
            success: function (response) {
                ...
                response.data.forEach(function (city) {
                    cityList.append(`
                     <div>
                        <button class="-link d-block col-12 text-left rounded-4 px-20 py-15 js-search-option getcity ">
                            <div class="d-flex">
                                <div class="icon-location-2 text-light-1 text-20 pt-4"></div>
                                <div class="ml-10">
                                    <div class="text-15 lh-12 fw-500 js-search-option-target city">${city.name}</div>
                                    <div class="text-15 lh-12 fw-500 citycode">${city.code},${city.name}</div>
                                </div>
                            </div>
                        </button>
                         </div>
  ...
  function liveSearchCountry() {
  const targets = document.querySelectorAll('.js-liverSearch')
  if (!targets) return
    let data1 = [];
    $.ajax({
        url: location.origin + '/NamoriTravels/GetCountries', // Replace with your endpoint
        method: 'GET',
        success: function (response) {
            let countryList = $('#countryList');
            countryList.empty(); // Clear previous data
            let Datalist = response.data;
            data1 = Datalist;
            Datalist.forEach(country => {
                countryList.append(`
                 <div>
                        <button class="-link d-block col-12 text-left rounded-4 px-20 py-15 js-search-option getcountry">
                            <div class="d-flex">
                                <div class="icon-location-2 text-light-1 text-20 pt-4"></div>
                                <div class="ml-10">
                                    <div class="text-15 lh-12 fw-500 js-search-option-target">${country.name}</div>
                                    <div class="text-14 lh-12 text-light-1 mt-5 country-id">${country.code}, ${country.name}</div>
                                </div>
                            </div>
                        </button>
                   </div>
  ...
  const showList = (searchTerm, resultsEl) => {
          ...
          <button class="-link d-block col-12 text-left rounded-4 px-20 py-15 js-search-option">
            <div class="d-flex">
              ...
              <div class="ml-10">
                ...
                <div class="text-14 lh-12 text-light-1 mt-5">${e.code}, ${e.name}</div>
              </div>
            </div>
          </button>
        `
    ...
    // Function to attach click listeners to country options
    const attachClickListeners = () => {
        const options = document.querySelectorAll('.js-search-option');
        options.forEach(option => {
            option.addEventListener('click', () => {
                const title = option.querySelector('.js-search-option-target').innerHTML;
                const search = option.closest('.js-liverSearch').querySelector('.js-search');
                search.value = title; // Set the input to the selected country name
                option.closest('.js-popup-window').classList.remove('-is-active'); // Hide the dropdown
            });
        });
    };
...
function initMap() {
  ...
  const contentString = `
    <div class="mapItem d-flex">
       <img src="~/dashboard/img/lists/hotel/1/1.png" alt="image" class="mapItem__img size-100 rounded-4">
...
const Calendar = (function() {
  ...
  function updateVars() {
    ...
    let getAllDaysInMonth = (month, year) => {
        let today = new Date();
        ...
        let initialMonthArray = Array.from(
            ...
            (_, i) => {
                let dayNum = i + 1;
                let isPastDate = !isCurrentMonth && (year < today.getFullYear() || (year === today.getFullYear() && dayNum < today.getDate()));
                return {
                    weekDay: weekDaysOrder[new Date(year, month - 1, dayNum).getDay()],
                    dayNum,
                    isClickable: isCurrentMonth || year > today.getFullYear() || !isPastDate // Allow current and future dates
                }
            }
        );
  ...
  function getFullYearDates(startDate, countOfMonthToTake) {
    ...
    let allYearMonths = []
      ...
      for (let i = 0; i < 10 * 12; i++) {
          let year = parseInt(date[0]) + Math.floor((parseInt(date[1]) - 1 + i) / 12);
          let month = (parseInt(date[1]) - 1 + i) % 12 + 1; // Adjust for 1-based month
            ...
            ${allYearMonths.map((month, i) => {
              return `
                <div class="swiper-slide">
                  ...
                  <div class="elCalendar__month" ${month.monthName}>
                    ...
                    <div class="elCalendar__body">
                      ${month.firstDates.map(el => `
                        <div
                          data-index="${globalIndexUp()}" data-week="${startMonth + i > 12 ? startYear + 1 : startYear}" data-month="${month.monthName.slice(0, 3)}"
                          class="elCalendar__sell -dark"
                        >
                          <span class="js-date" date-value="${month.monthName} ${startMonth + i > 12 ? startYear + 1 : startYear}">
                            ${el.dayNum}
                          </span>
                        </div>
                      `).join('')}
                      ...
                      ${month.initialMonthArray.map(el => `
                        <div
                          data-index="${globalIndexUp()}" data-week="${month.monthName.slice(0, 3)} " data-month="${startMonth + i > 12 ? startYear + 1 : startYear}"
