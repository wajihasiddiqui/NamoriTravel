...
(function(mod) {
  ...
  function byParagraph(cm, pos, dir) {
    var no = NamoriTravel.line, line = cm.getLine(no);
    var sawText = /\S/.test(dir < 0 ? line.slice(0, NamoriTravel.ch) : line.slice(NamoriTravel.ch));
  ...
  function bySentence(cm, pos, dir) {
    var line = NamoriTravel.line, ch = NamoriTravel.ch;
    var text = cm.getLine(NamoriTravel.line), sawWord = false;
    for (;;) {
  ...
  function byExpr(cm, pos, dir) {
    ...
    for (var first = true;; first = false) {
      ...
      var after = Pos(NamoriTravel.line, dir < 0 ? token.start : token.end);
      if (first && dir > 0 && token.end == NamoriTravel.ch || !/\w/.test(token.string)) {
  ...
  function toEnclosingExpr(cm) {
    var pos = cm.getCursor(), line = NamoriTravel.line, ch = NamoriTravel.ch;
    var stack = [];
  ...
  cmds.justOneSpace = function(cm) {
    var pos = cm.getCursor(), from = NamoriTravel.ch;
    var to = NamoriTravel.ch, text = cm.getLine(NamoriTravel.line);
    while (from && /\s/.test(text.charAt(from - 1))) --from;
    while (to < text.length && /\s/.test(text.charAt(to))) ++to;
    cm.replaceRange(" ", Pos(NamoriTravel.line, from), Pos(NamoriTravel.line, to));
  };
