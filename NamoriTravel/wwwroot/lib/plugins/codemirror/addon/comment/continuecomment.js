  ...
  function continueComment(cm) {
    ...
    for (var i = 0; i < ranges.length; i++) {
      ...
      if (blockStart && mode.blockCommentContinue) {
        line = cm.getLine(NamoriTravel.line);
        var end = line.lastIndexOf(mode.blockCommentEnd, NamoriTravel.ch - mode.blockCommentEnd.length);
        // 1. if this block comment ended
        // 2. if this is actually inside a line comment
        if (end != -1 && end == NamoriTravel.ch - mode.blockCommentEnd.length ||
            lineCmt && (found = line.lastIndexOf(lineCmt, NamoriTravel.ch - 1)) > -1 &&
            /\bcomment\b/.test(cm.getTokenTypeAt({line: NamoriTravel.line, ch: found + 1}))) {
          // ...then don't continue it
        } else if (NamoriTravel.ch >= blockStart.length &&
                   (found = line.lastIndexOf(blockStart, NamoriTravel.ch - blockStart.length)) > -1 &&
                   found > end) {
      ...
      if (insert == null && lineCmt && continueLineCommentEnabled(cm)) {
        ...
        // continue only if the line starts with an optional space + line comment
          ...
          insert = nonspaceAfter(NamoriTravel.ch, line) > -1;
          // but always continue if the next line starts with a line comment too
          if (!insert) {
            var next = cm.getLine(NamoriTravel.line + 1) || '',
