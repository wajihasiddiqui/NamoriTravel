...
(function(mod) {
  ...
  function jumpBack(ts, cm) {
    var pos = ts.jumpStack.pop(), doc = pos && ts.docs[NamoriTravel.file];
    if (!doc) return;
    moveTo(ts, findDoc(ts, cm.getDoc()), doc, NamoriTravel.start, NamoriTravel.end);
  }
  ...
  function atInterestingExpression(cm) {
    ...
    if (tok.start < NamoriTravel.ch && tok.type == "comment") return false;
    return /[\w)\]]/.test(cm.getLine(NamoriTravel.line).slice(Math.max(NamoriTravel.ch - 1, 0), NamoriTravel.ch + 1));
  }
  ...
  function buildRequest(ts, doc, query, pos) {
    ...
    if (doc.changed) {
      if (doc.doc.lineCount() > bigDoc && allowFragments !== false &&
          ...
          doc.changed.from <= startNamoriTravel.line && doc.changed.to > query.end.line) {
  ...
  function makeTooltip(x, y, content, cm, className) {
    ...
    if (overlapY > 0) {
      ...
      if (curTop - height > 0) { // Fits above cursor
        node.style.top = (NamoriTravel.top - height) + "px";
      } else if (height > winH) {
        node.style.height = (winH - 5) + "px";
        node.style.top = (NamoriTravel.bottom - box.top) + "px";
      }
    }
    if (overlapX > 0) {
