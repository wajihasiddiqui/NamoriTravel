...
(function(mod) {
  ...
  function markChanges(editor, diff, type, marks, from, to, classes) {
    ...
    for (var i = 0; i < diff.length; ++i) {
      ...
      if (tp == DIFF_EQUAL) {
        var cleanFrom = NamoriTravel.line + (startOfLineClean(diff, i) ? 0 : 1);
        moveOver(pos, str);
        var cleanTo = NamoriTravel.line + (endOfLineClean(diff, i) ? 1 : 0);
    ...
    if (pending) markChunk(chunkStart, NamoriTravel.line + 1);
  }
  ...
  function moveOver(pos, str, copy, other) {
    var out = copy ? Pos(NamoriTravel.line, NamoriTravel.ch) : pos, at = 0;
    for (;;) {
      var nl = str.indexOf("\n", at);
      if (nl == -1) break;
      ++out.line;
