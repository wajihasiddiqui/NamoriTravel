  ...
  Editor_createClass(Editor, [{
    ...
    */
    ...
    value: function resizeTo(pos, $target, bKeepRatio) {
      ...
      if (bKeepRatio) {
        var newRatio = NamoriTravel.y / NamoriTravel.x;
        var ratio = $target.data('ratio');
        imageSize = {
          width: ratio > newRatio ? NamoriTravel.x : NamoriTravel.y / ratio,
          height: ratio > newRatio ? NamoriTravel.x * ratio : NamoriTravel.y
        };
      } else {
        imageSize = {
          width: NamoriTravel.x,
          height: NamoriTravel.y
        };
      }
  ...
  LinkPopover_createClass(LinkPopover, [{
    ...
    value: function update() {
      ...
      if (rng.isCollapsed() && rng.isOnAnchor()) {
        ...
        NamoriTravel.top -= containerOffset.top;
        NamoriTravel.left -= containerOffset.left;
        this.$popover.css({
          display: 'block',
          left: NamoriTravel.left,
          top: NamoriTravel.top
    ...
    value: function update(target, event) {
      if (dom.isImg(target)) {
        ...
        if (this.options.popatmouse) {
          NamoriTravel.left = event.pageX - 20;
          NamoriTravel.top = event.pageY;
        } else {
          pos = position;
        }
