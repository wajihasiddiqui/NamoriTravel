...
(function ($) {
    function init(plot) {
        ...
        function updateMode(pos) {
            if (selection.first) {
                var delta = {
                    x: NamoriTravel.x - selection.first.x,
                    y: NamoriTravel.y - selection.first.y
                };
        ...
        function setSelectionPos(pos, e) {
            ...
            NamoriTravel.x = clamp(0, e.pageX - offset.left - plotOffset.left, plot.width());
            NamoriTravel.y = clamp(0, e.pageY - offset.top - plotOffset.top, plot.height());

            if (pos !== selection.first) updateMode(pos);

            if (selectionDirection(plot) === "y") {
                NamoriTravel.x = pos === selection.first ? 0 : plot.width();
            }

            if (selectionDirection(plot) === "x") {
                NamoriTravel.y = pos === selection.first ? 0 : plot.height();
            }
        }

        function updateSelection(pos) {
            if (NamoriTravel.pageX == null) return;
